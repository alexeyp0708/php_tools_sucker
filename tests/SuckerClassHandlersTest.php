...
class SuckerClassHandlersTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function test_isset()
    {
        ...
        $this->assertTrue($handlers->isset('static_private_child_prop'));
        $this->assertFalse($handlers->isset('static_private_core_prop'));

        $handlers->setScope(Fixtures\CoreClass::class);
        $this->assertTrue($handlers->isset('static_private_core_prop'));
        $this->assertFalse($handlers->isset('static_private_subchild_prop'));
        $this->assertFalse($handlers->isset('static_private_child_prop'));
    }
    ...
    public function test_unset()
    {
        $handlers = self::$fixtures['handlers'];
        $handlers->setScope(null);
        $check = Assert::isError(fn() => $handlers->unset('static_private_subchild_prop'), 'Attempt to unset static property');
    ...
    public function test_sandbox($scope)
    {
        ...
        $handlers->setScope($scope);
        $tester = $this;
        $answer =& $handlers->sandbox(function (...$args) use ($tester, $scope) {
            $tester->assertSame($scope, self::class);
            return $args[0];
        }, ['hello']);
